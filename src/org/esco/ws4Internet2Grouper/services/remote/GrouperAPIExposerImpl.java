...
public class GrouperAPIExposerImpl implements IGrouperAPIExposer, InitializingBean {
	...
	private GrouperDTO fetchGrouperData(final Group g) {
	    return new GrouperDTO(g.getParentStem().getName(), 
	...
	private GrouperDTO fetchGrouperData(final Subject s) {
		return new GrouperDTO(s.getId(), s.getName(), s.getDescription());
	}
	 ...
	 */
	private GrouperDTO[] fetchGrouperData(@SuppressWarnings("unchecked") final Set memberships) {
	...
	/**
	 * Converts a subject to e member object.
	 * @param session The grouper session.
	...
	public boolean hasMember(final String groupName, final String subjectId) {
		...
		final Group group = fetchGroup(session, groupName);
		boolean member = false;

		if (group != null) {
			final Subject subject = fetchSubject(subjectId);
			member = group.hasAdmin(subject);
		}
	...
	/**
	 * Finds the description of a specified group from the data store.
	 * @param key The key of the group to look for.
	 * @return The group if it can be found, null otherwise.
	 * @see org.esco.ws4Internet2Grouper.services.remote.IGrouperAPIExposer#findGroup(java.lang.String)
	 */
	public GrouperDTO findGroup(final String key) {

		final GrouperSession session = createSession();
		final Group g = fetchGroup(session, key);
		GrouperDTO infos = null;

		if (g != null) {
			infos = fetchGrouperData(g);
		}
	...
	public GrouperDTO[] getMemberGroups(final String key) {
		...
		final GrouperSession session = createSession();
		final Group group = fetchGroup(session, key);
	...
	/**
	 * Gives the members of type subject or group of the group.
	 * @param key The name of the group.
	...
	public GrouperDTO[] getMembershipsForSubject(final String subjectKey) {
		...
		GrouperDTO[] infos = null;
		if (member != null) { 
			infos = fetchGrouperData(member.getMemberships());
		}
	 ...
	 * @return The list of the groups which the group identified by key belongs. 
	 * @see org.esco.ws4Internet2Grouper.services.remote.IGrouperAPIExposer
	 * #getMembershipsForGroup(java.lang.String)
	 */
	public GrouperDTO[] getMembershipsForGroup(@SuppressWarnings("unused")	final String key) {
		...
		if (group != null) {
			infos = fetchGrouperData(group.toMember().getImmediateMemberships());
		}
